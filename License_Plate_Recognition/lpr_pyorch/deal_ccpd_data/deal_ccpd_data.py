...
def fetch_plate_img(img_dir, save_dir):
    ...
    for root, dirs, files in os.walk(img_dir):
        for img in files:
            ...
            cv2.imwrite(os.path.join(save_dir, '{0}.jpg'.format(plate)), crop_img)
...
def generate_train_eval(img_dir, train_dir, eval_dir, eval_percent=0.05):
    ...
    for img in images:
        ...
        if val_num > 0:
            ...
            #file_util.copy_file(os.path.join(img_dir, plate), eval_dir)
            val_num -= 1
            eval_num += 1
            ...
            #file_util.copy_file(os.path.join(img_dir, palte), train_dir)
...
if __name__ == "__main__":
    ...
    verify_img(img_dir=save_dir_, error_img_save_dir=error_img_save_dir_)
    ...
    #step 4 : 生成训练-评估数据
    generate_train_eval(img_dir=save_dir_, train_dir=train_dir_, eval_dir=eval_dir_, eval_percent=0.03)
